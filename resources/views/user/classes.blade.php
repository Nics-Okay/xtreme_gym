...
@section('title', 'Class - Xtreme Gym World')
...
@section('main-content')
    ...
    <div class="user-content-container">
        ...
        <div class="user-main-content">
            <div class="main-section">
                @if($students->isEmpty())
                    @if($class_lists->isEmpty())
                        <p>No class available at the moment.</p>
                    @else
                                ...
                                <div class="class-item">
                                    <div class="section-upper">
                                        <div class="section-upper-left">
                                            <h3>Coach {{ $class_list->trainer }}</h3>
                                            <p>Instructor</p>
                                        </div>
                                        <div class="section-upper-right">
                                            <h3>{{ $class_list->name }}</h3>
                                            <p>
                                                {{ \Carbon\Carbon::parse($class_list->class_start)->format('F j, Y') }} - 
                                                {{ \Carbon\Carbon::parse($class_list->class_end)->format('F j, Y') }}
                                            </p>
                                        </div>
                                    </div>
                                    <div class="section-middle">
                                        <h2>₱ {{ $class_list->price }}.00</h2>
                                        <p>{{ $class_list->description }}</p>
                                        <p>{{ $class_list->schedule }}</p>
                                    </div>
                                    <div class="section-bottom">
                                        <p>Status: {{ $class_list->status }}</p>
                                    </div>
                                </div>
                            @endforeach
                        </div>
    ...
    <div id="modal" class="modal">
        <div class="modal-content">
            ...
            <form action="{{ route('user.availClass') }}" method="POST">
                @csrf
                <div class="form-group">
                    <div class="form-content">
                        ...
                        <select name="class" id="class" required>
                            ...
                            @foreach( $class_lists as $class_list)
                                <option value="{{ $class_list->id }}">{{ $class_list->name }} Class for {{ $class_list->price  }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
